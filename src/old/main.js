    ...
    if (global.race["pet"]) {
      ...
      if (change !== 0) {
        ...
        obj.popper.append(
          ...
          )}</span> <span class="has-text-${style}"> ${change}%</span></p>`
        );
      }
    }
...
function fastLoop() {
  ...
  if (global.race.species === "protoplasm") {
    ...
    if (smelterUnlocked()) {
      ...
      if (consume_coal > 0) {
        let max_operable = Math.max(
          0,
          Math.floor(
            global.resource.Coal.amount / (fuel_config.c_cost * time_multiplier)
          )
        );
    ...
    if (
      ...
      ["Neutronium", "Adamantite", "Bolognium", "Orichalcum"].forEach(function (
        res
      ) {
    ...
    if (global.resource.Infernite.display) {
      ...
      if (workers > 0) {
        ...
        let surveyor_delta =
          surveyor_base * tunneler * sensors * global_multiplier;
