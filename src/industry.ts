...
import { actions, checkCityRequirements, checkPowerRequirements } from './actions';
import { races, traits, fathomCheck } from './races';
import { atomic_mass } from './resources';
import { checkRequirements, checkSpaceRequirements, convertSpaceSector, planetName } from './space';
import { fortressTech } from './portal';
import { edenicTech } from './edenic';
...
/**
 * Render the smelter panel.
 *
 * The markup is React's now (components/SmelterPanel.tsx); this keeps the
 * container and hands it over. `bind` still selects between the two targets —
 * a selector for the Industry tab row, absent for the options modal — and the
 * ids the stylesheet hangs off follow from that, so it is passed through
 * rather than baked into the component.
 *
 * tooltip() and matText() below stay here. They read engine state to describe
 * what each fuel costs and what each output does, and they are handed to the
 * hook rather than imported, since they are local to this function.
 */
function loadSmelter(parent,bind){
    clearElement(parent);
    mountSmelter(bind ? $(bind)[0] : parent[0], {
        isModal: !bind,
        engine: { tooltip, matText, colorRange, trickOrTreat, easterEgg },
    });
    ...
    // The fuel and material descriptions are React popovers now, attached to
    // the rows themselves (components/SmelterPanel.tsx). Registering them here
    // bound them by selector on the line after the panel was drawn, which was
    // safe against jQuery and is not against React's asynchronous render.
}
