...
legacy.checkAffordable = checkAffordable;
legacy.craftCost = craftCost;
legacy.craftingRatio = craftingRatio;
legacy.races = races;
legacy.universe_types = universe_types;
legacy.seasonDesc = seasonDesc;
legacy.flib = flib;
...
function legacyInit() {
  ...
  if (global.race.species === 'protoplasm') {
    // Protoplasm state init (RNA display, prestige perks, universe setup)
    protoplasmStateInit();
    // Evolution tab is rendered by React EvolutionTab
  } else {
    // Non-protoplasm: legacy init for all tabs and DOM bindings
    initTabs();
    buildQueue();
    resizeGame();
    legacyDOMInit();
...
function protoplasmStateInit() {
    ...
    let grand_rank =
        ...
        global.stats.achieve.corrupted.l > 0
            ? Math.min(global.stats.achieve.corrupted.l, global.stats.feat['grandmaster'])
            : 0;
...
function legacyDOMInit() {
  ...
  popover(
    ...
    function (obj) {
      ...
      let total =
        100 + global.city.morale.unemployed + global.city.morale.stress;
    ...
    function (obj) {
      if (global.race.species === "protoplasm") {
        ...
        if (global.race["lone_survivor"]) {
          challenges =
            challenges +
            `<div>${loc("evo_challenge_lone_survivor_desc")}</div>`;
        }
...
const powerWarnings: Record<
  string,
  { warn: boolean; current?: number; max?: number }
> = {};
    ...
    [
      ...
      if (global[region][generator.s] && global[region][generator.s]["on"]) {
        ...
        if (p_on[generator.s] !== global[region][generator.s].on) {
          setPowerWarning(
            `#${region}-${generator.s} .on`,
            true,
            p_on[generator.s],
            global[region][generator.s].on,
          );
        } else {
          ...
          if (global[sup.a][area_structs[i]]) {
            ...
            if (operating * supportSize > remaining_support && !sup.oc) {
              ...
              setPowerWarning(
                ...
                true,
                operating,
                global[sup.a][area_structs[i]].on,
              );
            } else {
        ...
        if (global.interstellar[structs[i]]) {
          ...
          if (used_support + operating > global.interstellar.starport.s_max) {
            ...
            setPowerWarning(
              ...
              true,
              operating,
              global.interstellar[structs[i]].on,
            );
          } else {
            setPowerWarning(`#${id} .on`, false);
          }
        ...
        if (global.galaxy[gateway_structs[i]]) {
          ...
          if (operating > max_operating) {
            ...
            setPowerWarning(
              ...
              true,
              operating,
              global.galaxy[gateway_structs[i]].on,
            );
          } else {
            setPowerWarning(`#${id} .on`, false);
          }
        ...
        if (global.portal[purifier_structs[i]]) {
          ...
          if (used_support + operating > global.portal.purifier.s_max) {
            ...
            setPowerWarning(
              ...
              true,
              operating,
              global.portal[purifier_structs[i]].on,
            );
          } else {
            setPowerWarning(`#${id} .on`, false);
          }
        ...
        if (global.space[belt_structs[i]]) {
          ...
          if (
            ...
            setPowerWarning(
              ...
              true,
              operating,
              global.space[belt_structs[i]].on,
            );
          } else {
            setPowerWarning(`#${id} .on`, false);
          }
...
function fastLoopRender() {
  ...
  if (dirty.evolution) {
    // Evolution tab rendering handled by React EvolutionTab
    dirty.evolution = false;
  }
  if (dirty.tech) {
    drawTech();
    dirty.tech = false;
...
function midLoopRender() {
  ...
  if (global.race.species === "protoplasm") {
    ...
    if (
      global.city["s_alter"] &&
      $(`#popper[data-id="city-s_alter"]`).length > 0
    ) {
